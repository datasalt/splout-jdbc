...
public class SploutStatement implements Statement {

  private final SploutConnection conn;
  private final SploutClient client;

  public SploutStatement(SploutConnection conn) {
    this.conn = conn;
    client = new SploutClient(conn.getUrl());
  }

  private Pattern explicitKeyPattern = Pattern.compile("key=([^;]*)[;](.*)");
  /**
  ...
  public ResultSet executeQuery(String sql) throws SQLException {
    // First, check if query is preceded by an explicit key in the form key=myKey;select ...
    String key;
    Matcher m = explicitKeyPattern.matcher(sql);
    if (m.matches()) {
      key = m.group(1);
      sql = m.group(2);
    } else {
      // No explicit key. Let's try to autodetect
      key = composeKey(sql, conn.getKeys());
    }
    try {
      System.out.println("querying "+ sql + "with partition key " + key);
      QueryStatus status = client.query(conn.getTablespace(), key, sql, null);
      if (status.getError() != null) {
        throw new SQLException("Error: '" + status.getError() + "' on query: '" + sql +
            "' with partiion key: '" + key + "' for tablespace: '" + conn.getTablespace() + "'");
      }
      return new SploutResultSet(this, status);
    } catch (IOException e) {
  ...
  /**
   * Tries to extract the key from a given query
   */
  public static String composeKey(String sql, String[]keys) throws SQLException {
    StringBuilder sb = new StringBuilder();
    for(String key:keys) {
      sb.append(extractKey(sql,key));
    }
    return sb.toString();
  }

  private static String extractKey(String sql, String key ) throws SQLException {
    Pattern pattern = Pattern.compile(key + "\\s*=\\s*((\"([^\"]*)\")|('([^']*)')|(([0-9\\.,]*)))", Pattern.CASE_INSENSITIVE);
    Matcher matcher = pattern.matcher(sql);

    if (matcher.find()) {
      for(int group:new int[] {5, 3, 1}) {
        if (matcher.group(group) != null) {
          return matcher.group(group);
        }
      }
    }
    throw new SQLException("Imposible to extract key value for key '" + key +"' to be send to Splout SQL in query: '" + sql + "'");
  }
